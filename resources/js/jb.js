$(function() {
	...
	$('.jb-videos-slider-container').mouseenter(function() {
		$(this).addClass('jb-focus-in');
	}).mouseleave(function(){
		$(this).removeClass('jb-focus-in');
	});

	setInterval(function(){
		var count = jQuery('.jb-videos-slider-container .rvs-nav-item').length;

		if (!$('.jb-videos-slider-container').hasClass('jb-focus-in')) {
			if (!$('.jb-videos-slider-container .rvs-nav-item.rvs-active').hasClass('rvs-video-active')) {
				if ($('.jb-videos-slider-container .rvs-nav-item.rvs-active').attr('data-count') < count) {
					$('.jb-videos-slider-container .rvs-nav-item.rvs-active').next().click();
				} else {
					$('.jb-videos-slider-container .rvs-nav-item').first().click();
				}
			}
		}
	}, 5000);
